...
fn each_tick_reports_position_action_and_remaining_time() {
    ...
    assert!(stdout.contains("tick  1 | drone (0, 1) | action: north | budget remaining: 17"));
    assert!(stdout.contains("tick  2 | drone (0, 2) | action: north | budget remaining: 16"));
}
...
fn a_hazard_route_script_reports_the_hazard_telemetry_line_and_lower_final_budget() {
    ...
    assert!(output.status.success());
    assert!(stdout.contains("UPLINK REACHED"));
    assert!(stdout.contains("tick  6 | drone (4, 2) | action: north | budget remaining: 8"));
    assert!(stdout.contains("hazard triggered at (4, 2): -4 budget"));
}
