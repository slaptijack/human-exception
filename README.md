...
Run `human-exception` with no script path to launch the persistent, full-screen resistance console instead. The console currently exposes the navigation shell — Signals, Target, Controller, and Operation are reachable now with placeholder content; After Action is wired up but only reachable once an operation can actually conclude inside the console. Running an operation from inside the console is not implemented yet, so use the script-path form above for that. Press `F1` for contextual help, `F2`-`F5` to move between views, and `Ctrl+Q` to quit.
