...
Run `human-exception` with no script path to launch the persistent, full-screen resistance console instead. It opens on **Signals**, a stream of intercepted traffic, shared intelligence, and requests from other resistance cells; select a signal with the arrow keys and press `Enter` to inspect it. Inspecting the one actionable signal opens **Target**, a dossier of what is currently known about the opportunity; pressing `Enter` there commits to working it and opens **Controller** (still a placeholder — the in-console Lua editor arrives in a later build). **Operation** and **After Action** stay unreachable until a working set exists and an operation can actually run inside the console, which is not implemented yet, so use the script-path form above for that in the meantime. `F2`-`F5` jump directly to Signals/Target/Controller/Operation once their prerequisites are met; unavailable views are shown dimmed in the footer. Press `F1` for contextual help and a Lua reference, and `Ctrl+Q` to quit.
