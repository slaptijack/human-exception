...
Run `human-exception` with no script path to launch the persistent, full-screen resistance console instead. It opens on **Signals**, a stream of intercepted traffic, shared intelligence, and requests from other resistance cells; select a signal with the arrow keys — its detail follows automatically — and press `Enter` to open **Target**, a dossier of what is currently known about the opportunity, but only for the one signal marked `[OPEN]`. Pressing `Enter` in Target commits to working it and opens **Controller** (still a placeholder — the in-console Lua editor arrives in a later build). Once a working set exists, `F5` also opens **Operation**, though actually running a deployment there isn't implemented yet, so use the script-path form above for that in the meantime; **After Action** has no key bound to it yet, so it isn't reachable from inside the console this build. `F2`-`F4` jump directly to Signals/Target/Controller once their prerequisites are met; unavailable views are shown dimmed in the footer. Press `F1` for contextual help and a Lua reference, and `Ctrl+Q` to quit.
