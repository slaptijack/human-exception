...
mod tests {
    ...
    fn fresh_launch_through_first_contact_success_bootstrap_and_relaunch_is_one_coherent_lifecycle()
    {
        ...
        for (width, height) in [(120, 40), (150, 50)] {
            ...
            retry_events.extend(std::iter::repeat_n(press(KeyCode::Enter), 18)); // exhaust the budget
            let (state, _) = render_from(
                bootstrap_state(Some(&profile_path), Some(&intro_path)),
                width,
                height,
    ...
    /// 18-point budget in exactly 18 ticks (`crate::simulation`'s
    ...
    fn review_run_budget_exhaustion_lands_on_the_real_terminal_tick_with_evidence() {
        ...
        events.push(press(KeyCode::F(6)));
        events.push(press(KeyCode::Char(' '))); // pause
        events.extend(std::iter::repeat_n(press(KeyCode::Enter), 18)); // exhaust the budget
        events.push(press(KeyCode::F(5))); // Review Run
        ...
        for (width, height) in [(120, 40), (150, 50)] {
            ...
            assert!(buffer_contains(&terminal, "budget        0 / 18"));
            assert!(buffer_contains(
                &terminal,
    ...
    fn review_run_navigation_never_mutates_a_budget_exhausted_run() {
        ...
        events.push(press(KeyCode::F(6)));
        events.push(press(KeyCode::Char(' ')));
        events.extend(std::iter::repeat_n(press(KeyCode::Enter), 18));
        events.push(press(KeyCode::F(5)));
